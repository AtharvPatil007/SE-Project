...
class salesClass:
    def __init__(self, root):
        ...
        scrrolly=Scrollbar(sales_Frame, orient=VERTICAL)
        self.Sales_List=Listbox(sales_Frame, font=("goudy old style", 15), bg="white", yscrollcommand=scrrolly.set)
        scrrolly.pack(side=RIGHT, fill=Y)
        scrrolly.config(command=self.Sales_List.yview)
        ...
        scrrolly2=Scrollbar(bill_Frame, orient=VERTICAL)
        self.bill_area=Text(bill_Frame, bg="lightyellow", yscrollcommand=scrrolly2.set)
        scrrolly2.pack(side=RIGHT, fill=Y)
        scrrolly2.config(command=self.bill_area.yview)
